...
---------
.. versionadded::    24.04
   |br| Atualizado para rodar no SuPyPerson Pynóplia (04).
...
class Letra:
    ...
    def __init__(self, cena, x, y, lt="A"):
        OFF = 0x24B6 - ord("A")
        la = Aldeia.J.a(self.TRANSP, x=x + 15, y=y, w=LADO * 90 // 100, h=LADO * 90 // 100,
                        style={'font-size': f'{LADO // 2}pt'}, cena=cena)
...
class Piso:
    ...
    def __init__(self, cena, x, y, ai="NA"):
        ...
        tile = LADO
        az = azz[ai[1]]
        self.elt = Aldeia.J.a(self.ALDEIA, x=x, y=y, w=tile, h=tile, cena=cena,
                              style=dict(transform=f"rotate({az}deg)"))
...
class Aldeia:
    ...
    # COUNT = 2
    # ALDEIA = "https://i.imgur.com/UCWGCKR.png"
    def __init__(self, j, tela):
        Aldeia.J = j
        tile = 100
        self.shuffle_keys = self.ORDERED_KEYS[:]
        clique = j.c()
        self.cena = cena = j.c("https://i.imgur.com/sGoKfvs.jpg", direita=clique, tela=tela)
    ...
    def __todos(self):
        ...
        c = [Piso(cena, 600 + nk % 3 * 100, nk // 3 * 100, ai) for nk, ai in
             enumerate("LS JN LO JO FN JL GS JS GL".split())]
        #     for x in range(4) for y in range(3)]
        D = [Piso(cena, 600 + nk % 3 * 100, 300 + nk // 3 * 100, ai) for nk, ai in
             enumerate("AN JN BN DO EO DO KL HS KN".split())]
    ...
    def desafio0(self, solucao):
        c = [Piso(self.cena, 10 * LADO + i * LADO, j * LADO, ai) for j, linha in enumerate(solucao) for i, ai in
             enumerate(linha)]
...
def circus(vitollino, desafio, solucao, tela=None):
    Jogo, STYLE = vitollino.Jogo, vitollino.STYLE
    ...
    # Aldeia(Jogo())
    Aldeia(Jogo(), tela).circus(desafio, solucao, desafio)
